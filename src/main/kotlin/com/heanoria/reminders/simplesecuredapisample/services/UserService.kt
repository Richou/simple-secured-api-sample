...
class UserService (private val userRepository: UserRepository, private val roleRepository: RoleRepository) : UserDetailsService {
    ...
    fun getByEmail(username: String): User {
        ...
        return User(userEntity.id, userEntity.username, userEntity.email, userEntity.authorities.stream().map { userRole: UserRoleEntity -> Role(userRole.id, userRole.role.authority) }.toList())
    }

    fun createUser(userCreate: UserCreate): UserEntity? {
        ...
        return this.userRepository.save(mapper.map(userCreate))
    }
