...
class UserService (private var userRepository: UserRepository) : UserDetailsService {
    ...
    fun getByEmail(username: String?): User {
        ...
        val authorities = userEntity.authorities
        return User(userEntity.id, userEntity.username, userEntity.email, authorities.stream().map { userRole: UserRoleEntity? -> Role(userRole?.id, userRole?.authority) }.toList())
    }
